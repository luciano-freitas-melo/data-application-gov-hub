...
@dag(
    ...
    tags=["siape", "dados_escolares"],
)
def siape_dados_escolares_dag() -> None:
    """
    DAG que consome o endpoint consultaDadosEscolares da API SIAPE
    e armazena os dados de escolaridade dos servidores no schema 'siape'.
    """

    @task
    def fetch_and_store_dados_escolares() -> None:
        logging.info("Iniciando extração de dados escolares por CPF")
        ...
        for cpf in cpfs:
            try:
                ...
                resposta_xml = cliente_siape.call(
                    "consultaDadosEscolares.xml.j2", context
                )
                ...
                logging.info(f"Dado escolar inserido para CPF {cpf}")

            except Exception as e:
                logging.error(f"Erro ao processar CPF {cpf}: {e}")
                continue

    fetch_and_store_dados_escolares()


dag_instance = siape_dados_escolares_dag()
